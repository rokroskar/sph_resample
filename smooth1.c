...
int smTemperature(SMX smx)
{
  ...
  float *weights;
  int *cleared;
  int pi, nSmooth, i, pmi, nListSize, nZeroSmooth = 0.0;
  float ih2, fNorm, r2, rs;
  float fScatDens;

  if (smx->kd->bOutDiag) puts(">> smTemperature()");
  fflush(stdout);
  ...
  */
  weights = malloc(nListSize*sizeof(float));

  /*
  ** Allocate the cleared array - this keeps track of whether or not we've
  ** cleared a split particle's properties
  */
  cleared = malloc(nListSize*sizeof(int));
  for(i=0;i<smx->kd->nActive;i++) cleared[i]=0;
  ...
  for (pi=0;pi<smx->kd->nInitActive;++pi) {
    ...
    */
    nSmooth = smBallGather(smx,p[pi].fBall2,p[pi].r,nnList);
    if (nSmooth > 0 && p[pi].fTimeForm >= 0.0) {
    
      /*
      ** Distribute temperature and metalicity of parent particle
      */
      
      ih2 = 4.0/p[pi].fBall2;
      fNorm = M_1_PI*ih2*sqrt(ih2)*p[pi].fMass;
      for (i=0;i<nSmooth;++i) {
	r2 = nnList[i].fDist2*ih2;
	rs = 2.0 - sqrt(r2);
	...
	/* Check whether this particle has been cleared or not - if not, clear all 
	   the properties in order to add the smoothed properties
	*/
	...
	if(cleared[pmi] == 0)
	  {
	    ...
	    pm[pmi].fDensity = 0.0;
	    pm[pmi].fMetals = 0.0;
	    pm[pmi].fMFracOxygen = 0.0;
	    pm[pmi].fMFracIron = 0.0;
	    pm[pmi].CoolParticle.Y_HI = 0.0;
	    pm[pmi].CoolParticle.Y_HeI = 0.0;
	    pm[pmi].CoolParticle.Y_HeII = 0.0;
	...
	pm[pmi].fTemp               += rs/p[pi].fDensity*p[pi].fTemp;
	pm[pmi].u                   += rs/p[pi].fDensity*p[pi].u;
	pm[pmi].fMetals             += rs/p[pi].fDensity*p[pi].fMetals;
	pm[pmi].fMFracOxygen        += rs/p[pi].fDensity*p[pi].fMFracOxygen;
	pm[pmi].fMFracIron          += rs/p[pi].fDensity*p[pi].fMFracIron;	
	pm[pmi].CoolParticle.Y_HI   += rs/p[pi].fDensity*p[pi].CoolParticle.Y_HI;
	pm[pmi].CoolParticle.Y_HeI  += rs/p[pi].fDensity*p[pi].CoolParticle.Y_HeI;
	pm[pmi].CoolParticle.Y_HeII += rs/p[pi].fDensity*p[pi].CoolParticle.Y_HeII;
	
	pm[pmi].fWeights            += rs/p[pi].fDensity;

	pm[pmi].fTimeCoolIsOffUntil = fmax(pm[pmi].fTimeCoolIsOffUntil, \
					   p[pi].fTimeCoolIsOffUntil);
	if(pm[pmi].fWeights == 0) fprintf(stderr, "bad weights in for loop\n");
  ...
  for (pmi=0; pmi<smx->kd->nActive;++pmi) {
    if(pm[pmi].fWeights == 0) fprintf(stderr, "%d\t%f\n", pm[pmi].iParentOrder, 
				      sqrt(p[pm[pmi].iParentOrder].fBall2)/2.0);
    else {
    
      pm[pmi].fTemp               /= pm[pmi].fWeights;
      pm[pmi].fMetals             /= pm[pmi].fWeights;
      pm[pmi].fMFracOxygen        /= pm[pmi].fWeights;
      pm[pmi].fMFracIron        /= pm[pmi].fWeights;
      
      pm[pmi].CoolParticle.Y_HI   /= pm[pmi].fWeights;
      pm[pmi].CoolParticle.Y_HeI  /= pm[pmi].fWeights;
      pm[pmi].CoolParticle.Y_HeII /= pm[pmi].fWeights;	

...
struct chkHeader kdReadTipsyCheckpoint(KD kdg, KD kds, KD kdd, FILE *fp)
{
...
/* int kdDivideStars(KD kds, KD kds_old) */
/* { */
/*   int i, nNew=0.0, nOld, iold=0; */
/*   PINIT *p, *p_new, *p_old; */
  
/*   p = kds->pInit; */

/*   for(i=0;i<kds->nParticles;i++) if(p[i].fTimeForm >= 0.0) nNew++; */
  
/*   nOld = kds->nParticles - nNew; */
  
/*   kds_old->nParticles = nOld; */
  
/*   /\* allocate arrays *\/ */
/*   kds_old->pInit = (PINIT *)malloc(kds_old->nParticles*sizeof(PINIT)); */
/*   p_new = (PINIT *)malloc(nNew*sizeof(PINIT)); */

/*   p_old = kds_old->pInit; */

/*   for(i=0;i<kds->nParticles;i++)  */
/*     { */
/*       if(p[i].fTimeForm >= 0.0)  */
/* 	{ */
/* 	  p_new[i].iOrder = i+kdg->nGas+kdg->nDark; */
/* 	  p_new[i].fMass = cp.fMass; */
/* 	  p_new[i].fSoft = cp.fSoft; */
/* 	  p_new[i].u = cp.u; */
/* 	  p_new[i].fMetals = cp.fMetals; */
/* 	  p_new[i].CoolParticle = cp.CoolParticle; */
/* 	  p_new[i].fTimeCoolIsOffUntil = cp.fTimeCoolIsOffUntil; */
/* 	  p_new[i].fTimeForm = cp.fTimeForm; */
/* 	  p_new[i].fMassForm = cp.fMassForm; */
/* 	  if(cp.fTimeForm > 0) */
/* 	    assert(cp.fMassForm > 0); */
/* 	  p_new[i].fNSN = cp.fNSN; */
/* 	  p_new[i].fMFracOxygen = cp.fMFracOxygen; */
/* 	  p_new[i].fMFracIron = cp.fMFracIron; */
/* 	  p_new[i].iGasOrder = cp.iGasOrder; */
